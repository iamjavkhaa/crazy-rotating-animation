...
// users = [{ner: 'javkhlan', utas: 89808962}, {ner: 'sarah', utas: 89674999}]
let users = [{}];
// localStorage.users = JSON.stringify(users);
// let usersBase = JSON.parse(localStorage.usersBase);
...
function submit() {     
    ...
    if(    ( onep.style.color === 'blue') 
        ...
        users[0].lastName = document.querySelector('#lname').value;
        users[0].firstName = document.querySelector('#fname').value;
        users[0].phone = document.querySelector('#number').value;
        users[0].email = document.querySelector('#email').value;
        users[0].password = document.querySelector('#password').value;
        localStorage.usersBase = JSON.stringify(JSON.parse(localStorage.usersBase) = JSON.parse(localStorage.usersBase).push(users));
        ...
        alert('зөв бөглөнө үү');
    }
}
document.querySelector('#lastBtn').addEventListener('click', submit);
